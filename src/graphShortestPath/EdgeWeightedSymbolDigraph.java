...
/******************************************************************************
 *  Modified version based on Symbol Graph.java
 *  reads data from a csv file that lists the two airports in the first two
 *  columns and the weight in the third column.
...
public class EdgeWeightedSymbolDigraph {
    ...
    private EdgeWeightedDigraph digraph;             // the underlying graph
    ...
    public EdgeWeightedSymbolDigraph(String filename, String delimiter) {
        ...
        while (!in.isEmpty()) { //only the first two columns include cities
        ...
        // line to all others
        digraph = new EdgeWeightedDigraph(st.size());
        in = new In(filename);
        while (in.hasNextLine()) {
            String[] a = in.readLine().split(delimiter);
            int v = st.get(a[0]);
